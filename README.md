...
- Balls: `●` is the active layer only, never a status. Idle draft is `◐` (left half) in meta `#8b8e93`. Open idle is `○` paper. Review is `◎`. Queued is `◌`. Active keeps status ink and becomes `●` (draft you are on is gray `●`). Fail > review > approved > open > draft > queued > merged. Selected stack is `▶` plus paper name — the row is not washed. Chain is packed (`○-◐-◎`). No logo rainbow, no filled necklace.
